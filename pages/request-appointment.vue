<template>
  <section class="formComp">
    <div class="richText__Text">
      <p>CONTACT US</p>
      <h3>Send A Message To Southern Vascular of Panama City, LLC</h3>
      <p>If you have any questions, concerns, or comments regarding Southern Vascular of Panama City, LLC, please fill out
        the short contact form below.</p>
    </div>
    <form id="appointmentForm" class="formComp__Form" name="appointment" method="POST" @submit.prevent="handleSubmit"
      ...
      <div class="formComp__input" v-for="(input, i) in form" :key="i">
        ...
        <!--Checkbox-->
        <input :name="input.name" v-if="input.type === 'checkbox'" type="checkbox" />

        <!--Select-->
        <select :name="input.name" v-if="input.type === 'select'">
          <option :value="option" v-for="(option, i) in input.options" :key="i">{{ option }}</option>
        </select>
        <!--Textarea-->
        ...
        <div class="input__Radio" v-if="input.type === 'radio'">
          <div class="form__Radio" v-for="(radio, i) in input.options" :key="i">
            <input :name="input.name" type="radio" :value="radio.value" :id="radio.id" />
